...
var star = new Array(),
    ...
    h = "0123456789abcdef",
    col1 = new Array(h2d(backcolor.substr(0, 2)), h2d(backcolor.substr(2, 2)), h2d(backcolor.substr(4, 2))),
    col2 = new Array(h2d(starcolor.substr(0, 2)), h2d(starcolor.substr(2, 2)), h2d(starcolor.substr(4, 2)));

function h2d(x) {
    return (16 * (h.indexOf(x.substr(0, 1)))) + (h.indexOf(x.substr(1, 1)));
}

function d2h(x) {
    return h.substr(parseInt(x / 16), 1) + h.substr((x % 16), 1);
}

function newStar(x) {
    star[x] = new Array(0, Math.random() * (2 * Math.PI), Math.random() * (dist / 4));
}
...
function starsT() {
    ...
    for (n = 0; n < numstars; n++) {
        if (star.length < numstars) {
            newStar(n);
            document.write('<DIV id="star' + n + '" style="position:absolute;font-family:' + fontface + ';">' + startext + '</DIV>');
        }
        star[n][0] += speed;
        star[n][1] += spin;
